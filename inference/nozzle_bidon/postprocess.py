"""Postproceso YOLOv8 Ultralytics (salida tipica 1 x (4+nc) x N; nc=2 Bidon/Pico)."""
...
def postprocess_yolov8_ultralytics(
    ...
    """
    Devuelve (xyxy, scores, class_ids) en espacio del tensor de entrada.

    ``pred``: (1, 4+nc, N) o (4+nc, N); nc=2 para Bidon/Pico.
    """
    if pred.ndim == 3:
        pred = pred[0]
    pred = pred.T
    ...
    class_ids = np.argmax(cls_prob, axis=1).astype(np.int64)
