"""
Export nozzle_bidones RKNN v7 (RK3568) con hybrid INT8.
...
Salida:
  Yolo-Weights/yolov8n_nozzle_bidones_v7.rknn
  models/yolov8n_nozzle_bidones_v7.rknn  (deploy placa)
...
from prepare_nozzle_calib_v7 import build_calib_v7  # noqa: E402
...
def _ensure_calib() -> Path:
    ...
    if path.is_file() and path.stat().st_size > 0:
        ...
        return path
    print(f"--> Generando calibracion v7 ({nc.RKNN_INPUT_SIZE} stretch)...")
    n = build_calib_v7(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export nozzle_bidones RKNN v7 hybrid INT8."
    )
    parser.add_argument(
        "--full-int8",
        action="store_true",
    ...
    parser.add_argument(
        "--step2-only",
        action="store_true",
        help="Reutiliza artefactos step1 en rknn_build_v7/ (patch FP16 + step2).",
    )
    parser.add_argument(
        "--dataset",
        type=Path,
        default=None,
        help="dataset.txt calibracion (default rknn_nozzle_v7_dataset.txt).",
    )
    ...
    if not dataset_path.is_file():
        raise SystemExit(
            ...
            "Corre: python yolo_train/prepare_nozzle_calib_v7.py"
        )
