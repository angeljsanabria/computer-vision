"""
Prepara imagenes de calibracion INT8/hybrid para nozzle RKNN v3.

Genera parches 640x640 stretch (mismo preproceso que inference/nozzle en placa:
  stretch_bgr_to_rknn_input -> RGB uint8 NHWC con mean 0 / std 255 en RKNN).

RKNN lee JPG del dataset.txt; al ser ya 640x640 evita resize ambiguo en calibracion.

Uso (desde la raiz del repo):
  python yolo_train/prepare_nozzle_calib_v3.py
  python yolo_train/prepare_nozzle_calib_v3.py --max-images 200
"""
...
def _stretch_640_bgr(frame_bgr: np.ndarray) -> np.ndarray:
    return cv2.resize(
        frame_bgr,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Calib 640 stretch para RKNN nozzle v3.")
    ...
    print("Siguiente: python yolo_train/exp_yolov8n_nozzle_rknn_v3.py")
