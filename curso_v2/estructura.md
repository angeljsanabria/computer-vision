...
## Tema extra planificado: Simulador camara IP (HTTP + JPEG)

### Objetivo
Servidor en **Python** que simule de forma **simplificada** una camara IP (estilo ONVIF en el sentido de "camara en red"), **sin** implementar ONVIF completo, para **testing embebido**.

### Alcance previsto
- **Cliente:** ESP32-P4 como **cliente HTTP** que solicita **snapshots en JPEG** (GET/HEAD a un endpoint fijo, p. ej. `/snapshot.jpg`).
- **No incluye:** SOAP, WS-Discovery, perfiles ONVIF completos, RTSP obligatorio, autenticacion compleja (salvo que mas adelante se agregue como opcion).
- **Si incluye:** respuesta HTTP con `Content-Type: image/jpeg`, comportamiento similar a URLs de snapshot que muchas camaras IP exponen ademas del stack ONVIF.

### Momento sugerido en el curso
- Desarrollo del servidor en **PC** (misma red WiFi que el ESP32-P4).
- Integracion con **cliente HTTP en el ESP32-P4** alineada con **Fase 8** (red, setup ESP32-P4) o cuando se aborde captura desde "camara IP simulada".

### Estado
- **Planificado** en `curso/progreso.md` (fila Extra). La implementacion (script, dependencias, README) se hara cuando decidas avanzar el tema con el curso, con explicacion previa y codigo sugerido segun las rules.
